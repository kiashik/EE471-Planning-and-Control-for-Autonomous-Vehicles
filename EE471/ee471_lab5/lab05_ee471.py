...
cellWidth = 0.02
r_res = 0.05    # from lp LiDAR spec
r_max = 4       # from lp LiDAR spec
p_low = 0.4
p_high = 0.6
...
class OccupancyGrid:
    ...
    def update_polar_grid(self, r):
        ...
        for i in range(self.mPolarPatch):
            if r[i] > 0: # valid measuremnt
                self.polarPatch[i, 0:r[i]] = self.l_low     # free
                self.polarPatch[i, r[i]:r[i]+1] = self.l_high      # occ
                self.polarPatch[i, r[i]+1:] = self.l_prior  # unknown
            else:
                self.polarPatch[i, :] = self.l_prior        # unknown
    ...
    def generate_patch(self, psi):
        ...
        ndimage.map_coordinates(self.polarPatch, [phiPatch, rPatch], self.patch)
    ...
    def updateMap(self, x, y, psi, angles, distances):
        ...
        self.map[mapSlice] = np.clip(self.map[mapSlice], self.l_min, self.l_max)
